...
def get_coord_maps(size=128):
    ...
    xx_channel = xx_channel.unsqueeze(-1) # 2d matrix(1,size,size,1) with each row arange(size)

    yy_ones = torch.ones([1, size], dtype=torch.int32)
    yy_ones = yy_ones.unsqueeze(1)#different from xx_ones use 1 here

    yy_range = torch.arange(size, dtype=torch.int32).unsqueeze(0)
    yy_range = yy_range.unsqueeze(-1)#different from xx_ones use 1 here

    yy_channel = torch.matmul(yy_range, yy_ones)
    yy_channel = yy_channel.unsqueeze(-1) # 2d matrix(1,size,size,1) with each column arange(size)

    xx_channel = xx_channel.permute(0, 3, 1, 2)
    yy_channel = yy_channel.permute(0, 3, 1, 2) #both as size(1,1,16,16)

    xx_channel = xx_channel.float() / (size - 1)
    yy_channel = yy_channel.float() / (size - 1)

    xx_channel = xx_channel * 2 - 1
    yy_channel = yy_channel * 2 - 1 # map from (0,size-1) to (-1,+1)
...
if __name__ == '__main__':
    ...
    x = torch.rand(2, 1 + 64, 64, 64).cuda() #torch.Size([2, 3, 16, 16])
    y = coordconv(x) #torch.Size([2, 64, 8, 8])
    # import ipdb
    # ipdb.set_trace()
    #print(y.shape)